...
/* ─────────────────────────────────────────────────────────────────────────────
   TYPES
───────────────────────────────────────────────────────────────────────────── */
interface CostumeSkill { name: string; desc: string; tags?: string[]; }
interface CostumeData {
  id: string; name: string; rarity: string;
  emoji: string; color: string; description: string;
  skills: CostumeSkill[] | string[];
  featured?: boolean;                       // ← NEW: marks a costume as currently on banner
  spCost?: string; cooldown?: string;
  upgrade?: { min: string; recommended: string; ideal: string };
  tips?: string[];
}
...
interface CharacterJSON {
  ...
  stars: number; color: string; colorAlt?: string; emoji: string;
  rarity?: string;
  lore: string; overview?: string; gameplay?: string;
  keyInsight?: string;
  modeRatings: Record<string, number>;
  costumes: CostumeData[];
  investmentPriority?: number; investmentNote?: string;
  fhAppearance?: string;
  synergies?: { name: string; note: string; emoji: string }[];
  externalLinks?: { label: string; url: string; icon?: string }[];
  weapons?: any[]; gear?: any[]; statPriority?: string[];
  potentialUpgrades?: any[];
  teams?: any[]; tips?: any[]; benchmarks?: any[];
}

/* Flattened pill-row item (derived from each costume in each character) */
interface FeaturedCostume {
  charSlug: string; charName: string; costumeName: string;
  element: string; role: string; tier: string;
  banner: string; color: string; emoji: string;
  featured: boolean;
}

/* Flattened item for the "All Characters" marquee & search */
interface CharRosterItem {
  slug: string; name: string; element: string; archetype: string;
  tier: string; color: string; emoji: string; stars: number;
  costumeName: string; costumeEmoji: string;
}
...
/* CharacterDetail used inside the modal — assembled from the full JSON */
interface CharacterDetail {
  slug: string; name: string; stars: number; rank: string;
  element: string; archetype: string; color: string; emoji: string;
  lore: string; gameplay: string;
  modeRatings: Record<string, number>;
  costumes: { id: string; name: string; rarity: string; emoji: string; color: string; description: string; skills: string[]; }[];
}

/* ─────────────────────────────────────────────────────────────────────────────
   PROPS — data arrives from the server component wrapper (page.tsx)
───────────────────────────────────────────────────────────────────────────── */
interface BD2HubProps {
...
/* featured costumes are now determined by the `featured` flag inside each character's JSON */
...
───────────────────────────────────────────────────────────────────────────── */
function CharacterCard({ char, dark, onOpen }: { char: FeaturedCostume; dark: boolean; onOpen: (slug: string) => void }) {
  return (
    <motion.div whileHover={{ y: -6, scale: 1.04 }} transition={{ type: "spring", stiffness: 400, damping: 22 }} onClick={() => onOpen(char.charSlug)}
      ...
      <div className="flex items-start justify-between gap-2 mb-1">
        <p className={`font-bold tracking-tight text-sm ${dark ? "text-white" : "text-neutral-900"}`}>{char.charName}</p>
        <TierBadge tier={char.tier} />
      </div>
      <p className={`text-[11px] mb-2.5 ${dark ? "text-neutral-500" : "text-neutral-400"}`}>{char.costumeName}</p>
...
───────────────────────────────────────────────────────────────────────────── */
function CharacterPill({ char, dark, onOpen }: { char: CharRosterItem; dark: boolean; onOpen: (slug: string) => void }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div className="relative shrink-0 mx-2" style={{ width: 260, height: 92 }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={() => onOpen(char.slug)}>
      <motion.div animate={{ y: hovered ? -6 : 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }}
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className={`text-[11px] truncate mt-0.5 ${dark ? "text-neutral-500" : "text-neutral-400"}`}>{char.costumeName}</p>
...
/* ─────────────────────────────────────────────────────────────────────────────
   BOSS FIGHT SECTION
───────────────────────────────────────────────────────────────────────────── */
function BossSection({ dark, bossData }: { dark: boolean; bossData: Boss }) {
...
/* ─────────────────────────────────────────────────────────────────────────────
   MAIN PAGE
───────────────────────────────────────────────────────────────────────────── */
export default function BD2Hub({ characters, boss }: BD2HubProps) {
  ...
  /* ── Derive all roster items and featured costumes from the character JSONs ── */
  const { allCharacters, featuredCostumes, characterDetailMap } = useMemo(() => {
    const allChars: CharRosterItem[] = [];
    const featured: FeaturedCostume[] = [];
    const detailMap = new Map<string, CharacterDetail>();

    for (const ch of characters) {
      // Pick the first costume as the "default" display costume for the roster
      const mainCostume = ch.costumes[0];
      ...
      allChars.push({
        ...
        emoji:       ch.emoji,
        stars:       ch.stars,
        costumeName: mainCostume?.name ?? "",
        costumeEmoji: mainCostume?.emoji ?? ch.emoji,
      });

      // Build CharacterDetail for modal
      detailMap.set(ch.slug, {
        slug:      ch.slug,
        name:      ch.name,
        stars:     ch.stars,
        rank:      ch.tier,
        ...
        color:     ch.color,
        emoji:     ch.emoji,
        lore:      ch.lore ?? "Full guide coming soon.",
        gameplay:  ch.overview ?? ch.gameplay ?? "Full guide coming soon.",
        modeRatings: ch.modeRatings,
        costumes: ch.costumes.map((c) => ({
          id:          c.id,
          name:        c.name,
          rarity:      c.rarity,
          emoji:       c.emoji,
      ...
      // Gather featured costumes (those marked as `featured: true`)
      for (const cos of ch.costumes) {
        if (cos.featured) {
          featured.push({
            charSlug:    ch.slug,
            charName:    ch.name,
            costumeName: cos.name,
            element:     ch.element,
            role:        ch.archetype,
            tier:        ch.tier,
            banner:      cos.rarity,       // "Limited", "Standard", etc.
            color:       cos.color || ch.color,
            emoji:       cos.emoji || ch.emoji,
            featured:    true,
          });
        }
      }
    ...
    return { allCharacters: allChars, featuredCostumes: featured, characterDetailMap: detailMap };
  }, [characters]);

  const BOSS_FIGHT = boss;

  const openModal = (slug: string) => {
    const d = characterDetailMap.get(slug);
    if (d) setModalChar(d);
  };
  ...
  const filteredChars = allCharacters.filter((c) => {
    ...
    const matchQ = !q || c.name.toLowerCase().includes(q) || c.costumeName.toLowerCase().includes(q) || c.element.toLowerCase().includes(q) || c.archetype.toLowerCase().includes(q) || c.tier.toLowerCase().includes(q);
  ...
  const elements = [...new Set(allCharacters.map((c) => c.element))];
  ...
  return (
    <div className={`relative min-h-screen ${bg} ${text} cursor-none overflow-x-hidden`} style={{ fontFamily: "'DM Sans', 'Sora', system-ui, sans-serif" }}>
      ...
      {/* ── FEATURED COSTUMES (was: BANNER CHARACTERS) ── */}
      <section className={`border-y py-14 ${border} overflow-hidden`}>
        <div className="mx-auto mb-6 max-w-7xl px-6">
          ...
          <h2 className="text-2xl font-extrabold tracking-tight" style={{ fontFamily: "'Sora',sans-serif" }}>Featured Costumes</h2>
        </div>
        {featuredCostumes.length > 0 ? (
          featuredCostumes.length >= 5 ? (
            /* Enough cards for a smooth marquee */
            <div className="w-full overflow-hidden">
              <Marquee pauseOnHover gap={16} duration="25s" className="py-2">
                {featuredCostumes.map((c) => <CharacterCard key={`${c.charSlug}-${c.costumeName}`} char={c} dark={dark} onOpen={openModal} />)}
              </Marquee>
            </div>
          ) : (
            /* Too few for marquee — static centered row */
          ...
          <div className="mx-auto max-w-7xl px-6">
            <div className={`rounded-2xl border p-8 text-center ${card}`}>
              ...
              <p className={`text-sm font-semibold ${dark ? "text-white" : "text-neutral-900"}`}>No featured costumes right now</p>
              <p className={`text-xs mt-1 ${muted}`}>Check back when a new banner drops!</p>
            </div>
          </div>
        )}
          ...
          {filteredChars.length === 0 ? (
            ...
            <motion.div key="grid" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mx-auto px-6" style={{ width: "70%" }}>
              <div className="flex flex-wrap gap-2 pb-2">
                {filteredChars.map((c) => <CharacterPill key={c.slug} char={c} dark={dark} onOpen={openModal} />)}
              </div>
            </motion.div>
          ) : filteredChars.length < 6 ? (
            /* Too few characters for marquee — show static grid */
            <motion.div key="static-grid" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mx-auto px-6" style={{ width: "70%" }}>
                ...
                {rows.map((row, ri) => (
                  <div key={ri} style={{ overflowX: "hidden", overflowY: "visible", paddingTop: 10, paddingBottom: 4, marginTop: ri > 0 ? 4 : 0 }}>
                    <Marquee pauseOnHover gap={14} duration={rowDurations[ri]} reverse={ri === 1}>
                      {row.map((c) => <CharacterPill key={c.slug} char={c} dark={dark} onOpen={openModal} />)}
                    </Marquee>
                  </div>
                ))}
              </div>
      ...
      {BOSS_FIGHT && <BossSection dark={dark} bossData={BOSS_FIGHT} />}
